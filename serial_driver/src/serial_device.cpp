...
bool SerialDevice::Init() {
  ...
  if (OpenDevice() && ConfigDevice()) {
    ...
    while (true) {
      ...
      std::cout << "Connecting to find serial." << std::endl;
      cv::waitKey(3000);
    }
    CloseDevice();
    return false;
...
int SerialDevice::ReadUntil2(uint8_t *buf, uint8_t end1 , uint8_t end2,uint8_t max_len)
{
  ...
  uint8_t i = 0;
  if (NULL == buf) {
    //std::cout<<"empty msg:" <<std::endl;//debug
    return -1;
  } else {
    while(flag == 0)
    {
      ...
      if (*p == end1)
      {
        ...
        {
          flag = 0;
          // std::cout<<"Read a msg:" <<std::endl;//debug
          // std::cout<<"msg length:" << int(i) <<std::endl;//debug
          // std::cout<<"sizeof(p):" << sizeof(p) <<std::endl;//debug
          //error_rec_flag = 0;//debug
            return 1 ;
        }
      }
      p++;
      ...
      if (i>=max_len)
      {
        ...
        flag = 0;
        std::cout<<"too long msg:" << int(i) <<std::endl;//debug
        serial_loss++;
        if (serial_loss>500) {
          CloseDevice();
          Init();
        ...
        //error_rec_flag = i;//debug
        
        return int(i);
      }
    }
  }
  std::cout<<"wrong end msg:" <<std::endl;//debug
  //error_rec_flag = 1;//debug
  return 0;
}
