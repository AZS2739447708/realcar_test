...
void SerialDriverNode::gimbalCtrlCallback(const auto_aim_interfaces::msg::GimbalCtrl::SharedPtr msg)//云台控制回调函数
{
  ...
  int ret = device_ptr_->Write(send_buff_.get(), send_length);

  if (ret == send_length) {
    RCLCPP_WARN(this->get_logger(), "Serial write OK, len=%d", send_length);
  } else {
    RCLCPP_ERROR(this->get_logger(),
      "Serial write FAILED, send=%d, real=%d", send_length, ret);
}
...
void SerialDriverNode::publishTrajectoryMarker(float pitch, float yaw)                                //发布轨迹标志
{
  ...
  pitch +=pitch_bias;
  // 创建MarkerArray消息对象  fdb_rx_data_.data.yaw = msg->yaw;                                                             //串口通信数据赋值
...
{
  vx_ = 1;//static_cast<int16_t>(msg->linear.x);
  vy_ = 1;//static_cast<int16_t>(msg->linear.y);
  vw_ = 0;//static_cast<int16_t>(msg->angular.z);

  vx_ = std::clamp(vx_,(int16_t)-3.0,(int16_t)3);
  vy_ = std::clamp(vy_,(int16_t)-3.0,(int16_t)3);
  vw_ = std::clamp(vw_,(int16_t)-3.0,(int16_t)3);
